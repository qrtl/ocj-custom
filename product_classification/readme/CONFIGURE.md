The module ships the classification models and configuration UI with only a
small set of sample equipment classifications, so the taxonomy can be tailored
to each deployment (for example through a company-specific data module or the
external integration).

To maintain the available classifications:

1. Go to *Inventory > Configuration > Products* and open *Equipment
   Classifications*, *Equipment Sub-Classifications*, *Consumable
   Classifications* or *Manufacturers*.
2. Create the entries with a name and, preferably, a numeric code.
